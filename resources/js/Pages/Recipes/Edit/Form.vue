<template>
    <jet-form-section @submitted="updateRecipe">
        <template #title>
            Изменить информацию о рецепте
        </template>
        <template #description>
            Измените информацию о рецепте в базе
        </template>
        <template #form>
            ...
            <div class="col-span-6 sm:col-span-4">
                <div class="col-span-6 sm:col-span-4">
                    ...
                    <div class="mt-2" v-show="! imagePreview">
                        <img :src="recipe.image_url" alt="Current Profile Photo" class="h-20 w-20 object-cover">
                    </div>
                    <div class="mt-2" v-show="imagePreview">
                    ...
                    <div class="list-item flex-auto flex flex-row justify-between">
                        <div class="relative flex-3">
                            <select name="product_id[]"
                                ...
                                <option v-for="product in productsList" :key="product.id" :value="product.id" :selected="ingredients.id">{{
                                        product.name
                                    }}, {{ product.unit }}
                                </option>
                            </select>
...
export default {
    ...
    props: ['productsList','recipe','ingredients'],
    methods: {
        updateRecipe() {
            if (this.$refs.image) {
                this.form.image = this.$refs.image.files[0]
            }
            this.form.post(route('recipes.update',this.recipe), {
                preserveScroll: true
            })
        },
        selectNewImage() {
        ...
        addIngredient: function () {
            this.form.ingredients.push({product_id:''});
        },
